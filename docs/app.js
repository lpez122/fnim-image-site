...
function populateFocusCategorySelect() {
  ...
  if (!selectedCategoryIds.length) {
    focusCategorySelect.innerHTML = `<option value="">No categories selected</option>`;
    return;
  }
...
function updateOverviewStats() {
  ...
  if (layerCount) {
    layerCount.textContent = String(getLayersForModel().length);
  }
  if (categoryCount) {
    categoryCount.textContent = String(similarityData?.dataset?.categoryCount ?? categories.length);
  }
  if (sourceImageCount) {
...
function renderCategoryControls() {
  ...
  if (selectionCount) {
    selectionCount.textContent = `${selectedCount} selected / ${categories.length} total`;
  }
      ...
      return `
        ...
        >
          ...
          <span class="category-option-copy">
            ...
            <span class="category-option-meta">${category.sourceCount} images</span>
          </span>
        </button>
      `;
    })
...
function renderSummary() {
  ...
  const compareText = state.compareMode
    ? `Comparing ${state.selectedLayers.size} layers in ${model.label}.`
  ...
  summaryPanel.innerHTML = `
    <div class="summary-grid">
      <article class="summary-card">
        <h3>Current scope</h3>
        <p><strong>${categoryIds.length}</strong> categories selected.</p>
        <p><strong>${totalFolderImages}</strong> source images represented.</p>
        <p><strong>${totalPublicImages}</strong> public thumbnails visible for those categories.</p>
        <p><strong>${escapeHtml(categoryMap.get(state.focusCategoryId)?.label ?? "")}</strong> is the focus category.</p>
      </article>
      <article class="summary-card">
        <h3>Active layer</h3>
      ...
      <article class="summary-card">
        <h3>Full-dataset stats</h3>
        <p><strong>${formatScore(layerSummary?.meanOffDiagonal)}</strong> mean off-diagonal category-pair cosine.</p>
...
function renderNeighbors() {
  ...
  if (categoryIds.length < 2 || !model || !state.focusCategoryId) {
    neighborsGrid.innerHTML = `
      <div class="empty-state">
        Pick at least two categories to populate the nearest-neighbor view.
      </div>
    `;
    return;
  }
  ...
  return `
    <article class="neighbor-card">
      <div class="neighbor-header">
        ...
        <div class="matrix-metric">${neighbors.length} nearest categories</div>
      </div>

      <div class="focus-preview">
        <div>
          <p class="eyebrow">Focus category</p>
          <h3>${escapeHtml(focusCategory.label)}</h3>
          <p class="sample-category-meta">${focusCategory.sourceCount} source images</p>
        </div>
            ...
            (neighbor, index) => `
              <div class="neighbor-row">
                ...
                <div class="neighbor-copy">
                  ...
                  <span>${neighbor.category.sourceCount} source images</span>
                </div>
...
function renderMaps() {
  ...
  if (categoryIds.length < 2 || !model || !state.focusCategoryId) {
    mapGrid.innerHTML = `
      <div class="empty-state">
        Pick at least two categories to populate the 2D similarity projection.
      </div>
    `;
    return;
  }
...
function renderPairs() {
  ...
  if (categoryIds.length < 2 || !model) {
    pairGrid.innerHTML = `
      <div class="empty-state">
        Pick at least two categories to populate the pair ranking.
      </div>
    `;
    return;
  }
  ...
  return `
    <article class="pair-card">
      <div class="matrix-header">
        <div>
          <h3>${escapeHtml(layer.label)}</h3>
          <p class="matrix-subtitle">${escapeHtml(model.label)} pair ranking across the selected categories</p>
        </div>
...
function updateExplorationCaption() {
  ...
  if (state.activeExploration === "neighbors") {
    caption.textContent = state.compareMode
      ? `Showing nearest-neighbor rankings for ${layerIdsToRender.length} layers across ${categoryIds.length} selected categories.`
      : `Showing the nearest-neighbor view for ${categoryIds.length} selected categories.`;
    return;
  }

  if (state.activeExploration === "projection") {
    caption.textContent = state.compareMode
      ? `Showing 2D similarity projections for ${layerIdsToRender.length} layers across ${categoryIds.length} selected categories.`
      : `Showing the 2D similarity projection for ${categoryIds.length} selected categories.`;
    return;
  }

  caption.textContent = state.compareMode
    ? `Showing top, middle, and lowest pair rankings for ${layerIdsToRender.length} layers across ${categoryIds.length} selected categories.`
    : `Showing top, middle, and lowest pair rankings for ${categoryIds.length} selected categories.`;
}
...
function renderMatrices() {
  ...
  if (!advancedMatrix.open) {
    matrixGrid.innerHTML = `
      <div class="empty-state">
        Open this panel to view the clustered cosine matrix.
      </div>
    `;
    return;
  }
  ...
  if (categoryIds.length < 2 || !model) {
    matrixGrid.innerHTML = `
      <div class="empty-state">
        Pick at least two categories to render the clustered cosine matrix.
      </div>
    `;
    return;
  }
  ...
  return `
    <article class="matrix-card">
      <div class="matrix-header">
        <div>
          <h3>${escapeHtml(layer.label)}</h3>
          <p class="matrix-subtitle">${escapeHtml(model.label)} clustered cosine matrix</p>
        </div>
